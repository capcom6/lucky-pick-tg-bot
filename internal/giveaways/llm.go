...
func (l *LLM) MakeDescription(ctx context.Context, ga GiveawayDraft, image []byte) (string, error) {
	l.logger.Debug("making description",
		zap.String("description", ga.Description),
		zap.Time("publish_date", ga.PublishDate),
	)

	replacer := strings.NewReplacer(
		"{description}", ga.Description,
		"{publish_date}", ga.PublishDate.Format(time.DateOnly),
	)
					...
					Multi: []openrouter.ChatMessagePart{
						...
						},
						{
							...
							ImageURL: &openrouter.ChatMessageImageURL{
								URL: base64.StdEncoding.EncodeToString(image),
							},
						},
					},
				},
